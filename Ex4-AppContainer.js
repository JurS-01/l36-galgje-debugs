...
class AppContainer extends React.Component {
  ...
  guessLetterHandler = event => {
    const inputPresent = this.state.currentChosenLetter.length > 0
    const newLetter = !this.state.guessedLetters.includes(this.state.currentChosenLetter) // added: full line
    if (inputPresent && newLetter) {                                                      // adjusted if statement and seperated setState
